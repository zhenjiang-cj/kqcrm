...
<html>
  ...
  <form class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone)" action="<%=path%>/companyAction.do?method=doCxAdd" method="post" name="companyForm">
    ...
    <input type="hidden" name="prove_srl" id="prove_srl" value="<%=comForm.getProve_srl() %>">
		...
		<p>
			...
			<%
				if("".equals(comForm.getApply_order_id())){
					%>
						<a class="button" href="<%=path %>/coremain/company/upload_img.jsp?type=18&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getProve_srl() %>&operid=<%=comForm.getOperatorId() %>"
