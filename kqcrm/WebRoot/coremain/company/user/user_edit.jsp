...
<html>
  ...
  <form class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone)" action="<%=path%>/companyAction.do?method=doEditCompanyUser" method="post" name="companyForm">
    <input type="hidden" name="operatorId" id="operatorId" value="<%=comForm.getOperatorId() %>">
    <input type="hidden" name="company_user_sno" id="company_user_sno" value="<%=com.getCompany_user_sno() %>">
    <div class="formBar">
			<ul>
    ...
    <h1>企业用户信息</h1>
    	<p>
			<label>用户编号：</label>
			<input name="company_user_id" type="text"   maxlength="8"  class="required" value="<%=com.getCompany_user_id() %>"/>
		</p>
		<p>
			<label>用户名称：</label>
			<input name="company_user_name" type="text"  maxlength="256"  class="required" value="<%=com.getCompany_user_name() %>"/>
		</p> 
