...
<div class="pageContent">
  <form class="pageForm required-validate" onsubmit="return validateCallback(this)" action="<%=path%>/companyAction.do?method=doBaseEdit" method="post" name="baseForm">
  ...
  <input type="hidden" name="company_info_id" id="company_info_id" value="<%=comForm.getCompany_info_id() %>">
  	<div class="pageFormContent nowrap" layoutH="97">
				...
				<dt>企业名称：</dt>
					<dd>
						<input type="text" name="company_name"   class="required" value="<%=company.getCompany_name() %>" />
					</dd>
				</dl>
				<dl>
				<dt>公司类型：</dt>
				...
				<dt>所属区域：</dt>
					<dd>
						<select   name="company_region" id="company_region">
				...
				<dt>注册地址：</dt>
					<dd>
						<input type="text" name="regis_addr"   class="required"  size="100" value="<%=company.getRegis_addr() %>" />
				...
				<dt>成立日期：</dt>
					<dd>
						<input type="text" name="establishment_date"   class="date" readonly="true"   value="<%=company.getEstablishment_date() %>" /><a class="inputDateButton" href="javascript:;">选择</a>
				...
				<dt>企业资质图片：</dt>
					<dd>
						<a class="button" href="<%=path %>/coremain/company/show_imgview.jsp?type=1&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>" 
									target="dialog" rel="dlg_page1" title="展示企业资质图片" width="645" height="570">
									<span id="span_img_1">展示企业资质图片</span></a>
									<input type="hidden" name="aptitude_file_no" id="aptitude_file_no" class="required" value="<%=company.getAptitude_file_no() %>" />
						<font color="silver" style="text-align:right;font-size:16px;">（要求上传正本、副本）</font>
					</dd>
				</dl>
				<dl>
	<div class="divider"></div>
				...
				<dt>营业执照注册号：</dt>
					<dd>
						<input type="text" name="busi_license_no" id="busi_license_no" class="required"  value="<%=company.getBusi_license_no() %>"/>
						<a class="button" href="<%=path %>/coremain/company/show_imgview.jsp?type=2&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>"
									 target="dialog" rel="dlg_page1"   title="展示图片" width="645" height="570">
										<span id="span_img_2">展示图片</span></a>
				...
				<dt>组织机构代码号：</dt>
					<dd>
						 <input type="text" name="organization_no" id="organization_no"  value="<%=company.getOrganization_no() %>" />
					</dd>
				</dl>
				<dl>
				<dt>税务登记号：</dt>
					<dd>
						 <input type="text" name="tax_regist_no" id="tax_regist_no" value="<%=company.getTax_regist_no() %>" />
					</dd>
				</dl>
				<dl>
				<dt>注册资本：</dt>
					<dd>
						 <input type="text" name="regist_capital" id="regist_capital" class="required digits" value="<%=company.getRegist_capital() %>" />(元)
					</dd>
				</dl>
				<dl>
				<dt>开户银行：</dt>
					<dd>
						 <input type="text" name="open_bank" id="open_bank"   value="<%=company.getOpen_bank() %>"   /> 
					</dd>
				</dl>
				<dl>
				<dt>银行帐号：</dt>
					<dd>
						 <input type="text" name="bank_account" id="bank_account"  value="<%=company.getBank_account() %>"    /> 
					</dd>
				</dl>
				<dl>
				<dt>主营范围：</dt>
					<dd>
						 <textarea   name="main_range" id="main_range" rows="10" cols="100" ><%=company.getMain_range() %></textarea>
				...
				<dt>企业章程图片：</dt>
					<dd>
						 <a class="button" href="<%=path %>/coremain/company/show_imgview.jsp?type=3&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>" 
								target="dialog" rel="dlg_page1"  title="展示图片" width="645" height="570">
										<span id="span_img_3">展示图片</span></a>
				...
				<dt>服务质量/服务收费制度：</dt>
					<dd>
						<a class="button" href="<%=path %>/coremain/company/show_imgview.jsp?type=4&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>"
								 target="dialog" rel="dlg_page1"  title="展示服务质量/服务收费制度图片" width="645" height="570">
										<span id="span_img_4">展示服务质量/服务收费制度图片</span></a>
...
<div class="divider"></div>		
		<h1>企业联系方式</h1>
				...
				<dt>联系人：</dt>
					<dd>
						 <input type="text" name="contacts_name" id="contacts_name" class="required"  value="<%=company.getContacts_name() %>"  />
					</dd>
				</dl>
				<dl>
				<dt>部门/职务：</dt>
					<dd>
						 <input type="text" name="dept" id="dept"   value="<%=company.getDept() %>"   />
					</dd>
				</dl>
				<dl>
				<dt>电子邮箱：</dt>
					<dd>
						 <input type="text" name="email" id="email" class="email" value="<%=company.getEmail() %>"   />
					</dd>
				</dl>
				<dl>
				<dt>企业网址：</dt>
					<dd>
						  <input type="text" name="company_website" id="company_website" class="url"  value="<%=company.getCompany_website() %>"  />
					</dd>
				</dl>
				<dl>
				<dt>手机：</dt>
					<dd>
						   <input type="text" name="msisdn" id="msisdn" class="phone" maxlength="11"  value="<%=company.getMsisdn() %>"   />
					</dd>
				</dl>
				<dl>
				<dt>联系电话：</dt>
				...
				<dt>传真：</dt>
					<dd>
						   <input type="text" name="fax" id="fax"   value="<%=company.getFax() %>"   />
					</dd>
				</dl>
				<dl>
				<dt>邮编：</dt>
					<dd>
						   <input type="text" name="zip_code" id="zip_code" class="digits"  value="<%=company.getZip_code() %>"  />
					</dd>
				</dl>
				<dl>
				<dt>办公地址：</dt>
					<dd>
						   <input type="text" name="office_addr" id="office_addr" class="required"  value="<%=company.getOffice_addr() %>"  />
					</dd>
				</dl>
