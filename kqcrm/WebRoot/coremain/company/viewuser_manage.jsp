...
<%
	...
	for(int i=0;i<userList.size();i++){
		...
		if(userinfo.getEmployee_type().equals("2")){
			//法人（董事长）
			user = userinfo;
		}
	}
...
<html>
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function(){
	     	...
	     	var url = '<%=path%>/companyAction.do?method=viewUserResume&employee_id=<%=comForm.getEmployee_id() %>';
	     	 $("#jbsxBoxma").loadUrl(url,{ data: {}, dialogId: '', callback: null });
			...
			//$("#div1").css({width:80%,height:iContentH});
			//$("#div2").css({width:15%,height:iContentH});
		});
  ...
  <body>
				...
				<div class="pageContent sortDrag" selector="h1"  >
					...
					<div id="navTab0" class="panel"  >
						<div class="pageContent">
							  <form class="pageForm required-validate" onsubmit="return validateCallback(this,navTabAjaxDone)" action="<%=path%>/companyAction.do?method=doUserManageEdit" method="post" name="userForm">
							    ...
							    <h1>总经理简况</h1>
									...
									<p>
										<label>职务：</label>
										总经理<input type="hidden" name="station" id="station"  value="5" readonly="true" />
			 								<input type="hidden" name="post" id="post"  value="1" readonly="true" />
									</p>
									<p>
										<label>职称等级：</label>
								...
								<!-- 简历管理 -->
											<div id="jbsxBoxma" class="unitBox" style="margin-left:10px;">
											</div>
						</div>
					</div>
    ...
    <div id="div2" style="width:19%;  float:left;margin-left:10px;margin-top:0px" >
    	<div id="navTab2"    style="overflow-y:auto;overflow-x:hidden;position:relative; ">
						<h1>
							相关证件
						</h1>
						<div   >
