...
<%
	...
	VerifiInfo ver = verifilist.get(0);
%>
...
<html>
  ...
  <form class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone)" action="<%=path%>/companyAction.do?method=doVerifiEdit" method="post" name="companyForm">
    <input type="hidden" name="report_srl" id="report_srl" value="<%=ver.getReport_srl() %>">
    ...
    <h1>验资报告</h1>
    	<p>
			<label>总资产：</label>
			<input name="full_capital" type="text" maxlength="16" class="required digits" value="<%=ver.getFull_capital() %>"/>(万元)
		</p>
		<p>
			<label>净资产：</label>
			<input name="net_assets" type="text" maxlength="16"  class="required digits" value="<%=ver.getNet_assets() %>"/> (万元)
		</p>
		<p>
			<label>其中货币资金：</label>
			<input name="money_proportion" type="text" max="100" class="required number" value="<%=ver.getMoney_proportion() %>"/>(%)
		</p> 
		<p>
			<label>验证机构：</label>
			<input name="report_org_name" type="text"  maxlength="128"  value="<%=ver.getReport_org_name() %>" />
		</p> 
		<p>
			<label>验资报告日期：</label>
			 <input type="text" name="report_date" id="report_date"   class="required date" readonly="true"   value="<%=ver.getReport_date() %>"  />
