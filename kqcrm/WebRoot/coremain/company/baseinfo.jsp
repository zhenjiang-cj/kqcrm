...
<div class="pageContent">
  <form class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);" action="<%=path%>/companyAction.do?method=doBaseSave" method="post" name="baseForm">
  ...
  <input type="hidden" name="company_info_id" id="company_info_id" value="<%=comForm.getCompany_info_id() %>">
  	...
  	<div class="pageFormContent nowrap" layoutH="97">
				...
				<dt>企业名称：</dt>
					<dd>
						<input type="text" name="company_name" maxlength="128"  class="required" />
					</dd>
				</dl>
				<dl>
				<dt>公司类型：</dt>
				...
				<dt>注册地址：</dt>
					<dd>
						<input type="text" name="regis_addr"  maxlength="128" size="100" class="required" />
				...
				<dt>成立日期：</dt>
					<dd>
						<input type="text" name="establishment_date"   class="required date" readonly="true" /><a class="inputDateButton" href="javascript:;">选择</a>
				...
				<dt>批准日期：</dt>
					<dd>
						<input type="text" name="approva_date" id="approva_date" class="date" readonly="true" /><a class="inputDateButton" href="javascript:;">选择</a>
				...
				if("".equals(comForm.getApply_order_id())){
					...
					<a class="button" href="<%=path %>/coremain/company/upload_img.jsp?type=1&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>" 
									target="dialog" rel="dlg_page1" title="上传企业资质图片" width="645" height="370">
									<span id="span_img_1">上传企业资质图片</span></a>
									<input type="hidden" name="aptitude_file_no" id="aptitude_file_no" class="required" />
	...
	<div class="divider"></div>
				...
				if("".equals(comForm.getApply_order_id())){
					%>
					<input type="text" name="busi_license_no" id="busi_license_no" maxlength="18" class="required alphanumeric" />
						<a class="button" href="<%=path %>/coremain/company/upload_img.jsp?type=2&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>"
									 target="dialog" rel="dlg_page1"   title="上传图片" width="645" height="370">
										<span id="span_img_2">上传图片</span></a>
									<input type="hidden" name="busi_license_file_no" id="busi_license_file_no" class="required" />
						<%
				} 
				%>	
				...
				<dt>组织机构代码号：</dt>
					<dd>
						 <input type="text" name="organization_no" id="organization_no" maxlength="9"  class="alphanumeric"  />
					</dd>
				</dl>
				<dl>
				<dt>税务登记号：</dt>
					<dd>
						 <input type="text" name="tax_regist_no" id="tax_regist_no" maxlength="18" class="alphanumeric" />
					</dd>
				</dl>
				<dl>
				<dt>注册资本：</dt>
					<dd>
						 <input type="text" name="regist_capital" id="regist_capital" maxlength="16" class="required digits"  />(元)
				...
				<dt>开户银行：</dt>
					<dd>
						 <input type="text" name="open_bank" id="open_bank"  maxlength="128"     /> 
					</dd>
				</dl>
				<dl>
				<dt>银行帐号：</dt>
					<dd>
						 <input type="text" name="bank_account" id="bank_account"   maxlength="30" class="alphanumeric" /> 
					</dd>
				</dl>
				<dl>
				<dt>主营范围：</dt>
					<dd>
						 <textarea   name="main_range" id="main_range" rows="10" cols="100" class="required"></textarea>
				...
				<dt>兼营范围：</dt>
					<dd>
						 <textarea   name="and_range" id="and_range" rows="10" cols="100"  ></textarea>
					</dd>
				</dl>
				<dl>
				<dt>企业章程图片：</dt>
					...
					%>
					 <a class="button" href="<%=path %>/coremain/company/upload_img.jsp?type=3&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>" 
								target="dialog" rel="dlg_page1"  title="上传图片" width="645" height="370">
									...
									<input type="hidden" name="company_articles_file_no" id="company_articles_file_no" class="required" />
						<%
				} 
				%>	
				...
				<dt>企业章程：</dt>
					<dd>
						<textarea  name="company_articles" id="company_articles" rows="12" cols="100" ></textarea> 
					</dd>
				</dl>
				<dl>
				<dt>企业简介：</dt>
					<dd>
						<textarea  name="company_introduction" id="company_introduction" rows="12" cols="100" ></textarea> 
					</dd>
				</dl>
				<dl>
				<dt>服务质量/服务收费制度：</dt>
				...
				if("".equals(comForm.getApply_order_id())){
					%>
						<a class="button" href="<%=path %>/coremain/company/upload_img.jsp?type=4&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=comForm.getCompany_info_id() %>&operid=<%=comForm.getOperatorId() %>"
								 target="dialog" rel="dlg_page1"  title="上传服务质量/服务收费制度图片" width="645" height="370">
										<span id="span_img_4">上传服务质量/服务收费制度图片</span></a>
									<input type="hidden" name="service_info_file_no" id="service_info_file_no" class="required" />
						<%
				} 
				%>	
...
<div class="divider"></div>		
		<h1>企业联系方式</h1>
				...
				<dt>联系人：</dt>
					<dd>
						 <input type="text" name="contacts_name" id="contacts_name" class="required" maxlength="15"/>
					</dd>
				</dl>
				<dl>
				<dt>部门/职务：</dt>
					<dd>
						 <input type="text" name="dept" id="dept"  maxlength="15" />
					</dd>
				</dl>
				<dl>
				<dt>电子邮箱：</dt>
				...
				<dt>企业网址：</dt>
					<dd>
						  <input type="text" name="company_website" id="company_website" maxlength="50" class="url" />
					</dd>
				</dl>
				<dl>
				<dt>手机：</dt>
					<dd>
						   <input type="text" name="msisdn" id="msisdn" maxlength="11"  class="digits" />
					</dd>
				</dl>
				<dl>
				<dt>联系电话：</dt>
					<dd>
						   <input type="text" name="contacts_no" id="contacts_no" maxlength="20"  class="required" />
					</dd>
				</dl>
				<dl>
				<dt>传真：</dt>
				...
				<dt>邮编：</dt>
					<dd>
						   <input type="text" name="zip_code" id="zip_code" class="digits" maxlength="6" />
					</dd>
				</dl>
				<dl>
				<dt>办公地址：</dt>
					<dd>
						   <input type="text" name="office_addr" id="office_addr" maxlength="128" class="required" />
					</dd>
				</dl>
