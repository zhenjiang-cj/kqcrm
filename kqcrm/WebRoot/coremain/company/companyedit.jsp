...
<html>
  <head>
	...
	jQuery(document).ready(function(){
	     	$('#investor_type').val(<%=com.getInvestor_type()%>);
	     	$('#investor_typec_child').val(<%=com.getInvestor_typec_child()%>);
	     	$('#investment_way').val(<%=com.getInvestment_way()%>);
		});
  ...
  <form class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone)" action="<%=path%>/companyAction.do?method=doCompanyInfoEdit" method="post" name="companyForm">
    <input type="hidden" name="investment_info_srl" id="investment_info_srl" value="<%=com.getInvestment_info_srl() %>">
    ...
    <h1>企业投资者信息</h1>
    	<p>
			<label>名称：</label>
			<input name="investor_name" type="text"   maxlength="10"  class="required" value="<%=com.getInvestor_name() %>"/>
		</p>
		<p>
			<label>机构代码或身份证号：</label>
			<input name="org_or_card_no" type="text"  maxlength="20"  class="alphanumeric"  value="<%=com.getOrg_or_card_no() %>"/>
		</p>
		<p>
			<label>投资者类型：</label>
		...
		<p>
			<label>注册资本：</label>
			<input name="regist_capital" type="text"  maxlength="16" class="digits" value="<%=com.getRegist_capital() %>"/> （元）
		</p>
		<p>
			<label>验资报告：</label>
				<%
				if("".equals(comForm.getApply_order_id())){
					%>
						<a class="button" href="<%=path %>/coremain/company/upload_img.jsp?type=17&companyid=<%=comForm.getCompany_info_id() %>&orderid=<%=com.getInvestment_info_srl() %>&operid=<%=comForm.getOperatorId() %>"
												 target="dialog" rel="dlg_page1"   title="上传验资报告" width="645" height="370">
												<span id="span_img_17">上传验资报告</span></a>
						<%
				} 
				%>	
						<a class="button" href="<%=path %>/coremain/company/show_img.jsp?type=17&companyid=<%=com.getInvestment_info_srl() %>&orderid=<%=com.getInvestment_info_srl() %>&operid=<%=comForm.getOperatorId() %>" 
									target="dialog" rel="dlg_page17" title="展示验资报告" width="745" height="570">
									<span id="show_img_17">展示验资报告</span></a>
