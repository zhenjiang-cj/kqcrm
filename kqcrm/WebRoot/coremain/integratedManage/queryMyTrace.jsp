...
<html>
  ...
  <body>
    <form id="pagerForm" method="post" action="<%=path%>/approvalFlowAction.do?method=queryMyTrace">
...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="<%=path%>/approvalFlowAction.do?method=queryMyTrace" method="post">
	<div class="searchBar">
		<table class="searchContent">
			...
			<tr>
				...
				<td>
					申请等级：
					<select id="applyGrade" name="applyGrade">
						<option value="-99">--请选择--</option>
...
<div class="pageContent">
	<table class="table" width="100%" layoutH="138" style="height: 500px; overflow: auto;">
		<thead>
			<tr>
		...
		<tbody>
			...
			<tr target="sid" rel="${list.apply_order_id}" style="background-color:#${list.color_tip};">
			    <td>${index.index+1}</td>
				...
				<!--<td>
					<c:if test="${list.task_id == 1013}">  
					<a href="<%=path %>/approvalFlowAction.do?method=downloadTemplateFile">下载证书</a>
					</c:if>
				</td> -->
			</tr>
			</c:forEach>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
		...
		jQuery("#applyGrade").val('<%=GlobalFunc.initStr(afForm.getApplyGrade(),"-99")%>');
