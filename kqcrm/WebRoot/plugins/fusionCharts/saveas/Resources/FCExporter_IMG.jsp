<%@page import="javax.imageio.stream.FileImageOutputStream" %><%@ page import="java.awt.image.BufferedImage" %><%@ page import="java.io.File" %><%@ page import="java.io.OutputStream" %><%@ page import="com.nl.util.chart.fusion.exporter.generators.ImageGenerator" %><%@ page import="com.nl.util.chart.fusion.exporter.beans.ChartMetadata" %><%@ page import="com.nl.util.chart.fusion.exporter.encoders.JPEGEncoder" %><%@ page import="com.nl.util.chart.fusion.exporter.encoders.BasicEncoder" %><%@ page import="com.nl.util.chart.fusion.exporter.FusionChartsExportHelper" %><%@ page import="com.nl.util.chart.fusion.exporter.ErrorHandler" %><jsp:useBean id="exportBean" scope="request" class="com.nl.util.chart.fusion.exporter.beans.ExportBean"/><%String action = (String) exportBean.getExportParameterValue("exportaction");
...
//	String pathToWebAppRoot = getServletContext().getRealPath("/");
        String pathToWebAppRoot = application.getRealPath("/");
        ...
        if (exportTargetWindow.equalsIgnoreCase("_self")) {
            response.addHeader("Content-Disposition", "attachment; filename=\"" + FusionChartsExportHelper.toUtf8String(fileName) + "\"");
